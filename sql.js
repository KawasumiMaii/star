...
db.serialize(() => {
    ...
    db.run("CREATE TABLE character (id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT UNIQUE, health INTEGER, attack INTEGER, defense INTEGER, speed INTEGER, energy INTEGER, destiny TEXT)");

    const stmt = db.prepare("INSERT INTO character (name, health, attack, defense, speed, energy, destiny) VALUES (?, ?, ?, ?, ?, ?, ?)");
    stmt.run("开拓者•毁灭", 1203, 620, 460, 100, 120, "毁灭");
    stmt.finalize();
    ...
    const lightConeStmt = db.prepare("INSERT INTO light_cone (name, health, destiny, attack, defense, description) VALUES (?, ?, ?, ?, ?, ?)");
